	...
	private class User {
				...
				while (run) {
					try {
						...
						try {
							...
							System.err.println("Chat server: IOException while" + "closing connection: "
									+ ioe.getMessage());
						}
		...
		private void send(String mess) {
			try {
				...
				DBG("User.send(): ConnectException: '" + e.getMessage() + "' while sending message '" + mess + "'");
				...
				while (true) {
					try {
						...
						if (message.substring(0, 6).equals("Hello:")) {
							User newUser;
							DBG("Fikk inn connection fra: " + message.substring(6, message.length()));
							users.add(newUser = new User(message.substring(6, message.length()), newConn));
